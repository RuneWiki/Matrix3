...
public class ForumAuthManager {
	...
	public static void registerAuth(final Account account, final String username, final String password) {
		...
		if (account.getForumAuthId() != -1) {
			LoginServerChannelManager.sendUnreliablePacket(account.getWorld(), LoginChannelsPacketEncoder.encodePlayerGameMessage(account.getUsername(), "Your forum account is already authorised.").getBuffer());
			return;
		}
	...
	public static void syncAuth(final Account account) {
		...
		if (account.getForumAuthId() != -1) {
			LoginExecutorManager.authsExecutor.schedule(new Runnable() {
				...
				public void run() {
					try {
						String link = Settings.WEB_API_LINK + "?world=eco&action=auth&gameuser=" + URLEncoder.encode(account.getUsername(), "UTF-8") + "&userid=" + account.getForumAuthId();
						URLConnection c = new URL(link).openConnection();
						...
						t.printStackTrace();
						LoginServerChannelManager.sendUnreliablePacket(account.getWorld(), LoginChannelsPacketEncoder.encodePlayerGameMessage(account.getUsername(), "An error occured while trying to verify your forum account auth key.").getBuffer());
					}
				}
			...
			if (extremeDonator)
				account.setRank(Account.RANK_EXTREME_DONATOR, true);
			LoginExecutorManager.authsExecutor.schedule(new Runnable() {
				@Override
				public void run() {
					LoginServerChannelManager.sendUnreliablePacket(account.getWorld(), LoginChannelsPacketEncoder.encodePlayerGameMessage(account.getUsername(), "You currently don't have have forum account authorised. Talk to closest oracle of dawn to authorize it.").getBuffer());
				}
			}, 5000, TimeUnit.MILLISECONDS);
