...
public abstract class Entity extends WorldTile {
	...
	public Entity getCurrentFaceEntity() {
		return lastFaceEntity >= 0 ? (lastFaceEntity >= 32768 ? World.getPlayers().get(lastFaceEntity - 32768) : World.getNPCs().get(lastFaceEntity)) : null;
	}
	...
	public boolean addWalkStep(int nextX, int nextY, int lastX, int lastY, boolean check) {
		...
		if (dir == -1)
			return false;
		if (check && !World.checkWalkStep(getPlane(), lastX, lastY, dir, getSize()) || (this instanceof NPC && !canWalkNPC(getX() + Utils.DIRECTION_DELTA_X[dir], getY() + Utils.DIRECTION_DELTA_Y[dir])))//double check must be done sadly cuz of npc under check, can be improved later to only check when we want
			return false;
		if (this instanceof Player) {
	...
	public boolean restoreHitPoints() {
		...
		/*	if (hitpoints > maxHp) {
			if (this instanceof Player) {
				...
				if (player.getPrayer().usingPrayer(1, 9) && Utils.random(100) <= 15)
					return false;
			}
	...
	public boolean needMasksUpdate() {
		return nextColour != null || nextFaceEntity != -2 || nextAnimation != null || nextGraphics1 != null || nextGraphics2 != null || nextGraphics3 != null || nextGraphics4 != null || (nextWalkDirection == -1 && nextRunDirection == -1 && nextFaceWorldTile != null) || !nextHits.isEmpty() || !nextHitBars.isEmpty() || nextForceMovement != null || nextForceTalk != null || needTargetInformationUpdate;
	}
	...
	public void loadMapRegions() {
		...
		int sceneChunksRadio = Settings.MAP_SIZES[mapSize] / 16; //16 cuz a chunk is 8. radio is half
	...
	public void setNextAnimation(Animation nextAnimation) {
		if (nextAnimation != null && nextAnimation.getIds()[0] >= 0)
			lastAnimationEnd = Utils.currentTimeMillis() + AnimationDefinitions.getAnimationDefinitions(nextAnimation.getIds()[0]).getEmoteTime() + (nextAnimation.getDelay() * 10);
		...
		if (hit.getDamage() > 0) {
			...
			WorldTasksManager.schedule(new WorldTask() {
				...
				public void run() {
					if(user.isDead())
						return;
					user.heal((int) (hit.getDamage() > 2000 ? (200 + ((hit.getDamage() - 2000) * 0.05)) : hit.getDamage() * 0.1), 0, 0, false);
				}
	...
	public boolean isBound() {
		return effectsManager.hasActiveEffect(EffectType.BOUND) || effectsManager.hasActiveEffect(EffectType.BINDING_SHOT);
	}
