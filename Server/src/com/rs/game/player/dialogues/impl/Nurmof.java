...
public class Nurmof extends Dialogue {
	...
	public void start() {
		...
		{ NPCDefinitions.getNPCDefinitions(npcId).name, "Greetings and welcome to my pickaxe shop. Do you want", "to buy my premium quality pickaxes?" }, IS_NPC, npcId, 9827);
	...
	public void run(int interfaceId, int componentId) {
		if (stage == -1) {
			...
			if (componentId == 2) {
				...
				{ player.getDisplayName(), "No, thank you." }, IS_PLAYER, player.getIndex(), 9827);
			} else {
				stage = 1;
				sendEntityDialogue(SEND_1_TEXT_CHAT, new String[]
				{ player.getDisplayName(), "Are your pickaxes better than other pickaxes, then?" }, IS_PLAYER, player.getIndex(), 9827);
			}
		} else if (stage == 1) {
			stage = -2;
