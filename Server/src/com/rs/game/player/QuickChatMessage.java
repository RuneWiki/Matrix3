...
public class QuickChatMessage extends PublicChatMessage {
	...
	public QuickChatMessage(Player player, QuickChatOptionDefinition option, long[] data) {
		super(null, 0x8000);
		this.option = option;
	...
	public static void writeQuickChatData(Player player, OutputStream message, QuickChatOptionDefinition option, long[] data) {
		if (option.dynamicDataTypes != null) {
			for (int i = 0; i < option.dynamicDataTypes.length; i++) {
				QuickChatOptionDefinition.QuickChatStringType type = option.getType(i);
				long value = 0;
				switch (type.id) {
				...
				case 10: // Item search (tradeable?)
					...
					break;
				case 2:
					// There doesn't seem to be any options that use this (in 667), although it is a valid id
					if(Settings.DEBUG)
						System.out.println("unknown quickchat option type 2 " + data[i]); // it is supposed to send/recv 4 bytes
					break;
				case 4: // Skill lookup, client sends 1 byte but always 0?
					value = player.getSkills().getLevelForXp(option.staticData[i][0]);
					break;
				case 6: // Slayer assignment lookup
					if (option.staticData[i][1] == 395) { // This should always be true as there is only one such option (in 667)
						value = 0; // TODO: player.getSlayer... //as per datamap 1563
						break;
					}
					// Only option in 667
					if(Settings.DEBUG)
						System.out.println("Unknown slayer lookup: " + option.staticData[i][1]);
					break;
				case 7: // There doesn't seem to be any options that use this, although it is a valid id
					if(Settings.DEBUG)
						System.out.println("unknown quickchat option type 7 " + option.staticData[i][0]); // server is supposed to send 1 byte, and has 1 static data id, no data received
					break;
				case 8: // 'Rank' lookup
					...
					default:
						...
						if(Settings.DEBUG)
							System.out.println("unkown option type 8 " + option.staticData[i][0]);
						break;
					}
					break;
				case 9: // 'Points' lookup, these seem to be more prone to changes than type 8 (both send an int back)
					switch (option.staticData[i][0]) {
					case 4562:
					...
					case 9067:
						...
						break;
					case 10057:
						value = player.getDominionTower().getKilledBossesCount();
						break;
					case 10118:
						value = player.getDominionTower().getMaxFloorClimber();
						break;
					case 10119:
						value = player.getDominionTower().getMaxFloorEndurance();
						break;

					default:
						// These are all options in 667
						if(Settings.DEBUG)
							System.out.println("unkown option type 9 " + option.staticData[i][0]);
						break;
					}
					break;
				case 11: // Skill lookup for next level & xp table datamap
					value = player.getSkills().getLevelForXp(option.staticData[i][1]);
					break;
				case 12:
				...
				case 14: // Soul wars data lookup
					...
					default:
						...
						// These are all the options in 667
							System.out.println("unknown soul wars option " + option.staticData[i][0]);
						break;
					}
					break;
