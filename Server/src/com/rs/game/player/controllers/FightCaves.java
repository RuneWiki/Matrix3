...
public class FightCaves extends Controller {
	...
	{ 1044, 1050, 1051 }; //updated to rs3
	

	public void playMusic() { //same here
		player.getMusicsManager().playMusic(getCurrentWave() == 63 ? 339 : selectedMusic);
	}
	...
	public static void enterFightCaves(Player player) {
		...
		player.getControlerManager().startControler("FightCavesControler", Settings.DEBUG ? WAVES.length : 1); // start
		// at
		// wave
		// 1
	}
	...
	/*
	 * logout or not. if didnt logout means lost, 0 logout, 1, normal, 2 tele, 4win
	 */
	public void exitCave(int type) {
		...
		if (type == 0)
			...
			player.getTimersManager().removeTimer(type == 4 ? RecordKey.FIGHT_CAVES : null);
			if (type == 1 || type == 4) {
				...
				if (type == 4) {
					...
					player.getInventory().addItemDrop(6529, 16064 * Settings.getDropQuantityRate(), outside);
					...
					tokkul *= Settings.getDropQuantityRate(); // 10x more
