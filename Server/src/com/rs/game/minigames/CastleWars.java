...
public final class CastleWars {
	...
	public static void createPlayingGame() {
		...
		GameExecutorManager.fastExecutor.scheduleAtFixedRate(playingGame, 60000, 60000);
					...
					for (Player player : playing[i].toArray(new Player[playing[i].size()])) {
						...
						if (winner != -1) {
							...
							if (winner == -2) {
								...
								player.getInventory().addItem(CW_TICKET, 1 * Settings.getDropQuantityRate());
							} else if (winner == i) {
								player.getPackets().sendGameMessage("You won.");
								player.getInventory().addItem(CW_TICKET, 2 * Settings.getDropQuantityRate());
							} else
	...
	public static void refreshTimeLeft(Player player) {
		player.getVarsManager().sendVarOld(380, playingGame == null ? 0 : playingGame.minutesLeft - (player.getControlerManager().getControler() instanceof CastleWarsPlaying ? 5 : 0));
	}
	...
	private static class PlayingGame extends TimerTask {
		...
		public void refresh(Player player) {
			player.getVarsManager().sendVarBitOld(143, flagStatus[SARADOMIN]);
			player.getVarsManager().sendVarBitOld(145, score[SARADOMIN]);
			player.getVarsManager().sendVarBitOld(153, flagStatus[ZAMORAK]);
			player.getVarsManager().sendVarBitOld(155, score[ZAMORAK]);
		}
