...
public class GraveStone extends NPC {// 652 - gravestone selection interface
		...
		if (clean) {
			...
			GameExecutorManager.slowExecutor.schedule(new Runnable() {
				...
				public void run() {
					try {
						for (FloorItem item : floorItems) {
							boolean isBeginningAccount = getPlayer() != null && getPlayer().isBeginningAccount();
	...
	public void sendGraveInscription(Player reader) {
		reader.getInterfaceManager().sendCentralInterface(GRAVE_STONE_INTERFACE);
		reader.getVarsManager().sendVarBitOld(4191, graveStone == 0 ? 0 : 1);
		if (ticks <= 50) // if the grave is almost broken
			reader.getPackets().sendIComponentText(GRAVE_STONE_INTERFACE, 5, "The inscription is too unclear to read.");
		else if (reader == getPlayer())
			reader.getPackets().sendIComponentText(GRAVE_STONE_INTERFACE, 5, "It looks like it'll survive another " + (ticks / 100) + " minutes. Isn't there something a bit odd about reading your own gravestone?");
		else
			reader.getPackets().sendIComponentText(GRAVE_STONE_INTERFACE, 5, inscription);
	}
