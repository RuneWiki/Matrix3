...
public class TheDarkOracle extends NPC {
	...
	static {
		...
		/* chaotics hweens phats and santas */
		dList[Drops.UNCOMMON].add(new Drop(1050, 1, 1));
		dList[Drops.UNCOMMON].add(new Drop(1053, 1, 1));
		dList[Drops.UNCOMMON].add(new Drop(1055, 1, 1));
		dList[Drops.UNCOMMON].add(new Drop(1057, 1, 1));
		dList[Drops.UNCOMMON].add(new Drop(1038, 1, 1));
		dList[Drops.UNCOMMON].add(new Drop(1040, 1, 1));
		dList[Drops.UNCOMMON].add(new Drop(1042, 1, 1));
		dList[Drops.UNCOMMON].add(new Drop(1044, 1, 1));
		dList[Drops.UNCOMMON].add(new Drop(1046, 1, 1));
		dList[Drops.UNCOMMON].add(new Drop(1048, 1, 1));
		...
		dList[Drops.UNCOMMON].add(new Drop(18359, 1, 1));
	...
	public ArrayList<Entity> getPossibleTargets(boolean checkPlayers, boolean checkNpcs) {
		ArrayList<Entity> list = super.getPossibleTargets(checkPlayers, checkNpcs);
	...
	public Item sendDrop(Player player, Drop drop) {
		int size = getSize();
		Item item = ItemDefinitions.getItemDefinitions(drop.getItemId()).isStackable() ? new Item(drop.getItemId(), (drop.getMinAmount() * Settings.getDropQuantityRate()) + Utils.getRandom(drop.getExtraAmount() * Settings.getDropQuantityRate())) : new Item(drop.getItemId(), drop.getMinAmount() + Utils.getRandom(drop.getExtraAmount()));
		World.sendNews(player, "Oracle of Darkness dropped " + item.getAmount() + " x " + item.getName(), 1);
		World.addGroundItem(item, new WorldTile(getCoordFaceX(size), getCoordFaceY(size), getPlane()), null, false, 60);
		return item;
	}
			...
			public void run() {
				if (phase == 0) {
					...
					for (Entity trg : getPossibleTargets(true, true)) {
						if (trg == TheDarkOracle.this || !trg.withinDistance(TheDarkOracle.this, 3))
							continue;
						trg.applyHit(new Hit(TheDarkOracle.this, Utils.getRandom(50) + 100, HitLook.DESEASE_DAMAGE));
					}
				} else if (phase == 1) {
					phase = 2;
					setHitpoints(100000);
					...
					for (Entity trg : getPossibleTargets(true, true)) {
						if (trg == TheDarkOracle.this || !trg.withinDistance(TheDarkOracle.this, 3))
							continue;
						trg.applyHit(new Hit(TheDarkOracle.this, Utils.getRandom(200) + 100, HitLook.DESEASE_DAMAGE));
					}
				} else if (phase == 2) {
					phase = 3;
					setHitpoints(100000);
					for (Entity trg : getPossibleTargets(true, true)) {
						if (trg == TheDarkOracle.this || !trg.withinDistance(TheDarkOracle.this, 3))
							continue;
						trg.applyHit(new Hit(TheDarkOracle.this, Utils.getRandom(600) + 100, HitLook.DESEASE_DAMAGE));
					}
				} else {
					for (Entity trg : getPossibleTargets(true, true)) {
						if (trg == TheDarkOracle.this || !trg.withinDistance(TheDarkOracle.this, 3))
							continue;
						trg.applyHit(new Hit(TheDarkOracle.this, Utils.getRandom(800) + 100, HitLook.DESEASE_DAMAGE));
					}
					World.sendNews("Oracle of Darkness has been killed!", World.WORLD_NEWS);
