...
import com.rs.game.npc.corp.DarkEnergyCore;
	...
	public int attack(final NPC npc, final Entity target) {
		...
		for (Entity t : targets) {
			...
			if (Utils.colides(t.getX(), t.getY(), t.getSize(), npc.getX(), npc.getY(), npc.getSize())) {
				stomp = true;
				delayHit(npc, 0, t, getRegularHit(npc, getMaxHit(npc, Utils.random(5804) + 2000, NPCCombatDefinitions.MELEE, target)));
			}
		}
		if (stomp) {
		...
		if (style == 3 || style == 4) {
			...
			Projectile projectile = World.sendProjectileNew(npc, style == 1 ? tile : target, 1823 + style, 41, style == 1 ? 0 : 16, 10, 1.5d, 16, 0);
			int projectileCycles = Utils.projectileTimeToCycles(projectile.getEndTime()) - 1, damage = getMaxHit(npc, style == 2 ? 5000 : npc.getMaxHit(NPCCombatDefinitions.MAGE), NPCCombatDefinitions.MAGE, target);
			if (style == 0 && damage > 0 && target instanceof Player) {
				...
				for (int i = 0; i < 6; i++) {
					...
					if (!World.isTileFree(newTile.getPlane(), newTile.getX(), newTile.getY(), 1))
						continue;
					final Projectile subProjectile = World.sendProjectileNew(tile, newTile, 1824, 0, 0, projectile.getEndTime() + 5, 1, 30, 0);
					WorldTasksManager.schedule(new WorldTask() {
					...
					}, Utils.projectileTimeToCycles(subProjectile.getEndTime()) - 1);
					World.sendGraphics(npc, new Graphics(1806, subProjectile.getEndTime(), 0, 0, true), newTile);
				}
				return npc.getAttackSpeed();
			}
