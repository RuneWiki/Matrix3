...
public class ItemListDumper {
	...
	public ItemListDumper() throws IOException {
		...
		writer.close();
		file = new File("clientScriptMap.txt"); // = new File("information/itemlist.txt");
		...
		for (int i = 0; i < 100000; i++) {
			...
			if (map != null) {
				writer.append(i+" - '"+ClientScriptMap.getMap(i).keyType+"':'"+ClientScriptMap.getMap(i).valueType+"' - " + v + " - " + s + " - " + map.toString());
				writer.newLine();
				writer.flush();
			}
		}
