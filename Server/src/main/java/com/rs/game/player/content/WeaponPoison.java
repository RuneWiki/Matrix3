...
public class WeaponPoison {
	...
	public static boolean poison(Player player, Item item, int type, boolean dungeoneering) {
		...
		if (weapon == null)
			return false;
		int amt = item.getAmount() > Settings.getCraftRate() * 15 ? Settings.getCraftRate() * 15 : item.getAmount();
		...
		player.getInventory().addItem(dungeoneering ? Drinkables.DUNGEONEERING_VIAL : Drinkables.VIAL, 1);
