...
public class SawmillController extends Controller {
	...
	public void addPlank(int type, int amount) {
		...
		player.getVarsManager().sendVarBitOld(type == 5 ? 5008 : 4216 + type, job.planks[type]);
	}
	...
	private void refreshVars() {
		...
		if (job == null)
			return;
		player.getVarsManager().sendVarBitOld(5007, job.jobDetails == Jobs.QUICK ? 0 : 1);
		for (int i = 0; i < job.jobDetails.planks.length; i++)
			player.getVarsManager().sendVarBitOld(4208 + i, ((job.jobDetails.planks[i] - 5) / 5));
		for (int i = 0; i < job.planks.length; i++)
			player.getVarsManager().sendVarBitOld(i == 5 ? 5008 : 4216 + i, job.planks[i]);
	}
	...
	private void refreshPlanks() {
		player.getVarsManager().sendVarBitOld(4214, planks);
	}
