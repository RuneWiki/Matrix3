...
public class FairyRings {
	...
	public static boolean openRingInterface(Player player, WorldTile tile, boolean source) {
		if (checkAll(player)) {
			...
			if (source) {
				player.getInterfaceManager().sendCentralInterface(734);
	...
	private static void resetRingHash(Player player) {
		...
		for (int i = 0; i < 3; i++)
			player.getVarsManager().sendVarBitOld(2341 + i, 0);
	}
	...
	public static void handleDialButtons(final Player player, int componentId) {
		...
		for (int i = 0; i < 3; i++)
			player.getVarsManager().sendVarBitOld(2341 + i, locationArray[i] == 1 ? 3 : locationArray[i] == 3 ? 1 : locationArray[i]);
