	...
	public static boolean chopCanoeTree(Player player, final int configIndex) {
		player.getActionManager().setAction(new Action() {
			...
			public int processWithDelay(Player player) {
				...
				player.getVarsManager().sendVarBitOld(1839 + configIndex, 10);
				isComplete = true;
				stop(player);
				return 4;
			}
	...
	public static void createShapedCanoe(final Player player) {
		...
		WorldTasksManager.schedule(new WorldTask() {
			...
			public void run() {
				...
				player.unlock();
				player.getVarsManager().sendVarBitOld((int) player.getTemporaryAttributtes().get("canoe_config"), 11 + selectedCanoe);
				return;
			}
		}, 10);
	}
	...
	public static void deportCanoeStation(final Player player, int selectedArea) {
		...
		if (selectedArea != canoeArea) {
			if (selectedArea > (canoeArea + selectedCanoe + 1) || selectedArea < (canoeArea - selectedCanoe - 1)) {
				...
				player.closeInterfaces();
				player.getVarsManager().sendVarBitOld((int) player.getTemporaryAttributtes().get("canoe_config"), 0);
