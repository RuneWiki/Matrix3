package com.rs.game.player.dialogues.impl.dominionTower;
...
public class StrangeFace extends Dialogue {
	...
	public void start() {
		...
		{ player.getDisplayName(), "Hello?" }, IS_PLAYER, player.getIndex(), 9827);
	...
	public void run(int interfaceId, int componentId) {
		if (stage == -1) {
			...
			stage = 1;
			sendEntityDialogue(SEND_1_TEXT_CHAT, new String[]
			{ player.getDisplayName(), "Woah!" }, IS_PLAYER, player.getIndex(), 9827);
		} else if (stage == 1) {
			stage = 2;
			sendDialogue(
			...
			stage = 3;
			sendEntityDialogue(SEND_1_TEXT_CHAT, new String[]
			{ player.getDisplayName(), "Should I be?" }, IS_PLAYER, player.getIndex(), 9827);
		} else if (stage == 3) {
			stage = 4;
			sendDialogue(
			...
			stage = 5;
			sendEntityDialogue(SEND_1_TEXT_CHAT, new String[]
			{ player.getDisplayName(), "There are monsters in the tower?" }, IS_PLAYER, player.getIndex(), 9827);
		} else if (stage == 5) {
			stage = 6;
			sendDialogue(
			...
			stage = 7;
			sendEntityDialogue(SEND_1_TEXT_CHAT, new String[]
			{ player.getDisplayName(), "Impressive. So you control the tower?" }, IS_PLAYER, player.getIndex(), 9827);
			...
			sendEntityDialogue(SEND_1_TEXT_CHAT, new String[]
			{ player.getDisplayName(), componentId == 2 ? "Only the greatest warrior that ever lived!" : "I'm pretty handy with a weapon." }, IS_PLAYER, player.getIndex(), 9827);
			...
			stage = 13;
			sendEntityDialogue(SEND_1_TEXT_CHAT, new String[]
			{ player.getDisplayName(), "What?" }, IS_PLAYER, player.getIndex(), 9827);
		} else if (stage == 13) {
			stage = 14;
			sendDialogue(
			...
			{ player.getDisplayName(), "You mentioned that you were created by someone, but", "why?" }, IS_PLAYER, player.getIndex(), 9827);
		} else if (stage == 15) {
			stage = 16;
			...
			stage = 17;
			sendEntityDialogue(SEND_1_TEXT_CHAT, new String[]
			{ player.getDisplayName(), "Sorry? Are you alright?" }, IS_PLAYER, player.getIndex(), 9827);
		} else if (stage == 17) {
			stage = 18;
			sendDialogue(
			...
			stage = 19;
			sendEntityDialogue(SEND_2_TEXT_CHAT, new String[]
			{ player.getDisplayName(), "I'd thought that, as a guide, you'd be a little more", "welcoming." }, IS_PLAYER, player.getIndex(), 9827);
		} else if (stage == 19) {
			stage = 20;
