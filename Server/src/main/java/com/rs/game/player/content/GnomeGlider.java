...
public class GnomeGlider {
	...
	{ new WorldTile(2464, 3503, 3), new WorldTile(2845, 3501, 0), new WorldTile(3322, 3431, 0), new WorldTile(3284, 3213, 0), new WorldTile(2973, 2969, 0), new WorldTile(2545, 2972, 0) };
	...
	public static void sendGlider(final Player player, final int index, final boolean isReturning) {
		player.getVarsManager().sendVarOld(153, CONFIGS[isReturning ? 1 : 0][index]);
		...
		FadingScreen.fade(player, DELAY[index] + 1, new Runnable() {
			...
			public void run() {
				...
				player.getVarsManager().sendVarOld(153, 0);
			}
		});
	}
}
