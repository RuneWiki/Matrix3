...
public final class QuestManager implements Serializable {
	...
	public void checkCompleted() {
		...
		if (!completedQuests.contains(Quests.RECIPE_FOR_DISASTER) && player.getSkills().hasRequiriments(Skills.COOKING, 70, Skills.MAGIC, 59, Skills.THIEVING, 53, Skills.FISHING, 53, Skills.MINING, 52, Skills.CRAFTING, 50, Skills.FIREMAKING, 50, Skills.WOODCUTTING, 50, Skills.AGILITY, 48, Skills.RANGE, 40, Skills.HERBLORE, 45, Skills.FLETCHING, 10, Skills.SLAYER, 10, Skills.SMITHING, 40))
	...
	public void sendCompletedQuestsData(Quests quest) {
		...
		case PERIL_OF_ICE_MONTAINS:
			player.getVarsManager().sendVarBitOld(4684, 150);
			break;
		case NOMADS_REQUIEM:
			player.getVarsManager().sendVarBit(10095, 0);
			break;
		case KINGS_RANSOM:
			player.getVarsManager().sendVarBitOld(3910, 0);
			break;
		case FAIRY_RING_III:
			player.getVarsManager().sendVarBitOld(7856, 1);
			player.getVarsManager().sendVarBitOld(7857, 1);
			break;
		case THE_EYES_OF_GLOUPHIE:
			player.getVarsManager().sendVarBitOld(2503, 1);
			break;
		case AS_A_FIRST_RESORT:
			...
			player.getVarsManager().sendVarBitOld(4322, 1);
			break;
		default:
			break;
		}
