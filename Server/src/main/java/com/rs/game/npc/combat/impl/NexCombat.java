...
public class NexCombat extends CombatScript {
	...
	private void sendIceBarrage(Nex nex) {
		...
		for (final Entity possibleTarget : nex.getPossibleTargets()) {
			...
			int damage = getMaxHit(nex, NPCCombatDefinitions.MAGE, possibleTarget);
	...
	private void sendBloodAttack(final Nex nex, final Entity target) {
		...
		nex.setNextAnimation(new Animation(17414));
		nex.setNextGraphics(new Graphics(3375));
		Projectile projectile = World.sendProjectile(nex, target, 374, 20, 20, 20, 2, 0, 0);
				...
				if (Utils.random(2) == 0) {
					for (Entity entity : nex.getPossibleTargets()) {
						if (entity instanceof Player) {
							...
							if (distance > 30)
								distance = 30;
							player.getPackets().sendCSVarInteger(1435, (distance * 255 / 30));
						}
					}
				}
			}
	...
	private void sendMeleeAttack(Nex nex, Entity target) {
		...
		Hit hit = getMeleeHit(nex, getMaxHit(nex, NPCCombatDefinitions.MELEE, target));
		...
		WorldTasksManager.schedule(new WorldTask() {
			...
			public void run() {
				...
				if (ticks == 1) {
					...
					target.setNextForceMovement(new ForceMovement(nex, 2, Utils.getMoveDirection(nex.getCoordFaceX(target.getSize()) - target.getX(), nex.getCoordFaceY(target.getSize()) - target.getY())));
					nex.setTarget(target);
				} else if (ticks == 3) {
					target.setNextWorldTile(nex);
					target.getPackets().sendGameMessage("You've been injured and you cannot use " + (target.getPrayer().isAncientCurses() ? "protective curses" : "protective prayers") + "!");
					target.setStunDelay(5);
					target.getEffectsManager().startEffect(new Effect(EffectType.PROTECTION_DISABLED, 12));
				}
			}
		}, 0, 0);
	}
	...
	private void sendRangeAttack(final Nex nex) {
		...
		for (final Entity t : nex.getPossibleTargets()) {
			...
			Projectile projectile = World.sendProjectileNew(nex, t, 380, 25, 25, 32, 2, 0, 0);
			delayHit(nex, Utils.projectileTimeToCycles(projectile.getEndTime()) - 1, t, getRangeHit(nex, getMaxHit(nex, damage, NPCCombatDefinitions.RANGE, t)));
		}
	}
