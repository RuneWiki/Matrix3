...
public final class NPCCombat {
	...
	public boolean checkAll() {
		...
		if (!npc.isCantFollowUnderCombat()) {
			...
			if (npc.getAttackStyle() == NPCCombatDefinitions.MELEE && targetSize == 1 && size == 1 && Math.abs(npc.getX() - target.getX()) == 1 && Math.abs(npc.getY() - target.getY()) == 1 && !target.hasWalkSteps()) {
			...
			if (npc instanceof Nex) {
				...
				// unlike melee
				maxDistance = npc.isForceFollowClose() ? 0 : (npc.getCombatDefinitions().isFollowClose() || npc.getAttackStyle() == Combat.MELEE_TYPE ? 0 : 9);
				npc.resetWalkSteps();
