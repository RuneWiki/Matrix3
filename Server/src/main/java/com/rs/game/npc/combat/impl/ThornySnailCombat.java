...
public class ThornySnailCombat extends CombatScript {
	...
	public int attack(NPC npc, Entity target) {
		...
		if (usingSpecial) {// priority over regular attack
			...
			delayHit(npc, 1, target, getRangeHit(npc, getMaxHit(npc, 80, NPCCombatDefinitions.RANGE, target)));
			npc.setNextGraphics(new Graphics(1387));
		} else {
			npc.setNextAnimation(new Animation(8143));
			delayHit(npc, 1, target, getRangeHit(npc, getMaxHit(npc, 40, NPCCombatDefinitions.RANGE, target)));
		}
