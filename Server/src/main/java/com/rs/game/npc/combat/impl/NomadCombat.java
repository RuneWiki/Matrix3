...
public class NomadCombat extends CombatScript {
	...
	public int attack(final NPC npc, final Entity target) {
		...
		if (nomad.isMeleeMode()) {
			...
			delayHit(npc, 0, target, getRegularHit(npc, getMaxHit(npc, NPCCombatDefinitions.MELEE, target)));
			return 2;
		} else {
			if (target instanceof Player && nomad.useSpecialSpecialMove()) {
						...
						public void run() {
							if (!secondLoop) {
								...
								if (npc.clipedProjectile(target, false))
									delayHit(npc, Utils.projectileTimeToCycles(World.sendProjectileNew(npc, target, 1658, 30, 30, 25, 4, 0, 0).getEndTime()), target, getRegularHit(npc, 3000));
					...
					WorldTasksManager.schedule(new WorldTask() {
						...
						public void run() {
							if (!secondLoop) {
								...
								Projectile projectile = World.sendProjectileNew(npc, target, 2280, 30, 30, 25, 3, 0, 0);
								delayHit(npc, Utils.projectileTimeToCycles(projectile.getEndTime()), target, getRegularHit(npc, player.getMaxHitpoints() - 1));
								nomad.setCantFollowUnderCombat(false);
								nomad.setNextMovePerform();
								stop();
							}
				...
				Projectile projectile = World.sendProjectileNew(npc, target, 1657, 30, 30, 25, 5, 0, 0);
				delayHit(npc, Utils.projectileTimeToCycles(projectile.getEndTime()), target, getRegularHit(npc, damage));
				if (damage == 0)
					target.setNextGraphics(new Graphics(85, projectile.getEndTime(), 100));
			}
		}
		return npc.getAttackSpeed();
	}
