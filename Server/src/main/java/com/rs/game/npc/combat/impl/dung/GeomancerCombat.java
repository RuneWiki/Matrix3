...
public class GeomancerCombat extends CombatScript {
	...
	private void sendEarthBlast(NPC npc, Entity target, boolean multiAttack) {
		...
		for (Entity t : npc.getPossibleTargets()) {
			...
			delayHit(npc, 1, t, getMagicHit(npc, getMaxHit(npc, (int) (npc.getMaxHit(NPCCombatDefinitions.MAGE) * .7), NPCCombatDefinitions.MAGE, t)));
		}
	}
	...
	private void sendEntangle(NPC npc, Entity target) {
		...
		int damage = getMaxHit(npc, (int) (npc.getMaxHit(NPCCombatDefinitions.MAGE) * 0.95), NPCCombatDefinitions.MAGE, target);
			...
			if (damage > 0) {
				if (t instanceof Player) {
					...
					if (player.getPrayer().hasPrayersOn()) {
						...
						player.getEffectsManager().startEffect(new Effect(EffectType.PROTECTION_DISABLED, 25));
