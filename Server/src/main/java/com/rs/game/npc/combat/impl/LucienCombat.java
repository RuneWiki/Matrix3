...
public class LucienCombat extends CombatScript {
	...
	public int attack(final NPC npc, final Entity target) {
					...
					for (int[] tile : tiles.values()) {
						...
						for (Entity t : possibleTargets)
							if (t.getX() == tile[0] && t.getY() == tile[1])
								t.applyHit(new Hit(npc, Utils.random(4000) + 400, HitLook.REGULAR_DAMAGE));
					}
					stop();
				}
		...
		if (attackStyle == 0) { // normal mage move
			...
			npc.setNextAnimation(new Animation(11338));
			delayHit(npc, 2, target, getRangeHit(npc, getMaxHit(npc, 900, NPCCombatDefinitions.RANGE, target)));
			...
			delayHit(npc, 4, target, getMagicHit(npc, getMaxHit(npc, NPCCombatDefinitions.MAGE, target)));
