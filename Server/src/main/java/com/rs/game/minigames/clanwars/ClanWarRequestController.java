...
public final class ClanWarRequestController extends Controller {
	...
	public boolean processButtonClick(int interfaceId, int componentId, int slotId, int slotId2, int packetId) {
		...
		case 791:
			...
			case 143: // Accept
				...
				player.getVarsManager().sendVarBitOld(5293, 1);
				return false;
			}
		}
		return true;
	...
	public boolean canPlayerOption1(final Player target) {
		...
		player.setRouteEvent(new RouteEvent(target, new Runnable() {
			...
			public void run() {
				...
				if (target == null || target.hasFinished() || !target.withinDistance(player, 14) || !(target.getControlerManager().getControler() instanceof ClanWarRequestController)) {
				...
				if (target.getTemporaryAttributtes().get("clan_request_p") == player) {
					ClanWars clanWars = new ClanWars(target.getCurrentFriendsChat(), player.getCurrentFriendsChat());
	...
	private static boolean canRequest(Player player, Player target, boolean message) {
		...
		} else if (player.getCurrentFriendsChat() == target.getCurrentFriendsChat()) {
