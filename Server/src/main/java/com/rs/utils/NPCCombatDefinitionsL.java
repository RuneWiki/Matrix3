	...
	private static void loadUnpackedNPCCombatDefinitions() {
		...
		try {
			...
			while (true) {
				...
				npcCombatDefinitions.put(npcId, new NPCCombatDefinitions(hitpoints, attackAnim, defenceAnim, deathAnim, respawnDelay, attackGfx, attackProjectile, xp, follow, poisonImmune, poisonous, agressivenessType, agroRatio));
			}
			in.close();
			out.close();
		} catch (Throwable e) {
