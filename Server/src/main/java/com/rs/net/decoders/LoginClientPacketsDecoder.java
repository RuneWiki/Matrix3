...
public class LoginClientPacketsDecoder {
	...
	public static void decodeIncomingPacket(InputStream stream) {
		...
		if (Settings.DEBUG)
			Logger.log("Login client", "Received packet:" + LoginProtocol.getOpcodeName(opcode));
		switch (opcode) {
		case LoginProtocol.PACKET_PINGPONG:
			int num = stream.readUnsignedShort();
			Logger.log(LoginClientPacketsDecoder.class, "Pong! (" + num + ")");
			LoginClientChannelManager.sendReliablePacket(LoginChannelsPacketEncoder.encodePingPong(number++).getBuffer());
			break;
		...
		case LoginProtocol.PACKET_LOGINRSP:
			...
			if (status == 2) {
				PlayerHandlerThread.handleLoginResponse(sessionid, sessionuser, status, file_length, stream.readUnsignedByte(), stream.readUnsignedByte() == 1, stream.readUnsignedByte() == 1, stream.readUnsignedByte() == 1, stream.readUnsignedByte() == 1, stream.readUnsignedByte() == 1, stream.readUnsignedByte(), stream.readUnsignedByte() == 1, stream.readLong(), stream.readNullString(), stream.readNullString());
			} else {
				PlayerHandlerThread.handleLoginResponse(sessionid, sessionuser, status, file_length, 0, false, false, false, false, false, 0, false, 0, null, null);
			}
			break;
		...
		case LoginProtocol.PACKET_UPDATEPLAYERINTVAR:
			...
			if (player != null) {
				...
				case LoginProtocol.VAR_TYPE_RIGHTS:
					...
					break;
				case LoginProtocol.VAR_TYPE_RANKS:
					int id = var & 0xFFFF;
					boolean value = (var >> 16) == 1;
					if (id == 0)
						player.setDonator(value);
					else if (id == 1)
						player.setExtremeDonator(value);
					else if (id == 2)
						player.setSupporter(value);
					else if (id == 3)
						player.setGraphicDesigner(value);
					break;
				case LoginProtocol.VAR_TYPE_MESSAGEICON:
					player.setMessageIcon(var);
					break;
				default:
					Logger.log(LoginClientPacketsDecoder.class, "Unknown player var type!");
					break;
				}
			}
			break;
		...
		case LoginProtocol.PACKET_PLAYER_UPDATEFRIENDSINFO:
			...
			if (player != null && player.hasStarted()) {
				...
				while (stream.readByte() == 1) {
					...
					player.getFriendsIgnores().updateFriend(isStatusUpdate, displayName, previousDisplayName, world, fcRank, worldName);
				}
		...
		case LoginProtocol.PACKET_PLAYER_SENTPRIVATE:
			...
			if (isQc) {
				...
				if (player == null)
					player = World.getLobbyPlayer(username);
				if (player != null && player.hasStarted())
					player.getPackets().sendPrivateMessage(name, qcFileId, qcData);
			} else {
				message = stream.readString();
				player = World.getPlayer(username);
				if (player == null)
		...
		case LoginProtocol.PACKET_PLAYER_RECVPRIVATE:
			...
			if (isQc) {
				...
				if (player == null)
					player = World.getLobbyPlayer(username);
				if (player != null && player.hasStarted())
					player.getPackets().receivePrivateMessage(name, messageUid, iconId, qcFileId, qcData);
			} else {
				message = stream.readString();
				player = World.getPlayer(username);
				if (player == null)
				...
				if (player != null && player.hasStarted()) {
					if (player.isFilteringProfanity())
						message = Censor.getFilteredMessage(message);
					message = Utils.fixChatMessage(message);
					player.getPackets().receivePrivateMessage(name, messageUid, iconId, message);
				}
			}
			break;
		...
		case LoginProtocol.PACKET_PLAYER_FRIENDSCHATUPDATEINFO:
			...
			if (player == null)
				player = World.getLobbyPlayer(username);
			if (player != null && player.hasStarted() && stream.getRemaining() > 0) {
				...
				int selfRank = -1;
				OutputStream packet = player.getPackets().startFriendsChatChannel(ownerDisplayname, name, kickReq, membersCount);
				while (stream.readByte() == 1) {
					String displayName = stream.readString();
					...
					if (displayName.equals(player.getDisplayName()))
						selfRank = rank;
					player.getPackets().appendFriendsChatMember(packet, displayName, worldId, rank, worldName);
				}
				player.getPackets().endFriendsChatChannel(packet);

				if (player.getCurrentFriendsChat() == null || !player.getCurrentFriendsChat().getChannel().equals(ownerUsername))
		...
		case LoginProtocol.PACKET_PLAYER_FRIENDSCHATMSG:
			...
			if (isQc) {
				...
				if (player == null)
					player = World.getLobbyPlayer(username);
				if (player != null && player.hasStarted())
					player.getPackets().receiveFriendChatMessage(name, chatName, messageUid, iconId, qcFileId, qcData);
			} else {
				message = stream.readString();
				player = World.getPlayer(username);
				if (player == null)
				...
				if (player != null && player.hasStarted()) {
					if (player.isFilteringProfanity())
						message = Censor.getFilteredMessage(message);
					message = Utils.fixChatMessage(message);
					player.getPackets().receiveFriendChatMessage(name, chatName, messageUid, iconId, message);
				}
			}
			break;
		...
		case LoginProtocol.PACKET_STOREPURCHASE:
			...
			player = World.getPlayer(username);
			if (player != null)
				EconomyManager.processStorePurchase(player, item);
			break;
		default:
			Logger.log(LoginClientPacketsDecoder.class, "Received unknown packet from login server, op:" + opcode);
			break;
		}
