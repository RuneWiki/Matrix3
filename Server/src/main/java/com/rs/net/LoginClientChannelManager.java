...
import com.rs.net.decoders.LoginClientPacketsDecoder;
...
public class LoginClientChannelManager {
	...
	public static void init() throws SocketException {
		...
		boss.start();
		device.bind(new InetSocketAddress(Settings.LOGIN_CLIENT_ADDRESS_BASE.getPort() + Settings.WORLD_ID));
	}
	...
	private static void handleDevice() {
		...
		for (int i = 0; i < amount; i++) {
			...
			if (packet.getLength() < 9 || packet.getPort() != Settings.LOGIN_SERVER_ADDRESS_BASE.getPort() || !Arrays.equals(Settings.LOGIN_SERVER_ADDRESS_BASE.getAddress().getAddress(), packet.getAddress().getAddress())) {
				if (Settings.DEBUG)
					Logger.log(LoginClientChannelManager.class, "Login->Game packet was rejected");
			...
			case LoginProtocol.MSG_SYNC_LAST_RECEIVED:
				synchronized (queueLock) {
					...
					if (syncnum < out_syncnum || syncnum > (out_syncnum + outgoingQueueCount)) {
						// we don't have this packet buffered
						if (Settings.DEBUG)
							Logger.log(LoginClientChannelManager.class, "Sync error, reliable packet loss expected");
					...
					for (int a = 0; a < outgoingQueueCount; a++)
						writeReliablePacket(outgoingQueue[a], out_syncnum + (long) (a + 1));
					break;
				}
			case LoginProtocol.MSG_SYNC_LAST_SENT:
				in_syncnum = stream.readLong();
				if (Settings.DEBUG)
					Logger.log(LoginClientChannelManager.class, "Received incoming resync request, reliable packet loss expected");
				break;
			...
			case LoginProtocol.MSG_DATA_PACKET_U: // unreliable packet
				...
				if (datasyncnum != 0) {
					if (Settings.DEBUG)
						Logger.log(LoginClientChannelManager.class, "Received unreliable packet with wrong syncnum, does login use different encryption?");
					break;
				}
				LoginClientPacketsDecoder.decodeIncomingPacket(stream);
				break;
			default:
				if (Settings.DEBUG)
					Logger.log(LoginClientChannelManager.class, "Unknown packet received from login, does login use different encryption?");
				break;
			}
		}
	}
