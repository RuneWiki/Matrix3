...
public final class NPCCombatDefinitionsLNew {
	...
	public final static NPCCombatDefinitions DEFAULT_DEFINITION = new NPCCombatDefinitions(1, -1, -1, -1, 33, -1, -1, 0.2, true, false, false, false, 2);
	...
	private static void loadPackedNPCCombatDefinitions() {
		try {
			...
			while (buffer.hasRemaining()) {
				...
				npcCombatDefinitions.put(npcId, new NPCCombatDefinitions(hitpoints, attackAnim, defenceAnim, deathAnim, respawnDelay, attackGfx, attackProjectile, 0.2, false, false, false, agressivenessType == 1, agroRatio));
			}
			channel.close();
			in.close();
		} catch (Throwable e) {
	...
	private static void createData() {
		try {
			...
			BufferedWriter writer = new BufferedWriter(new FileWriter(new File("npcs.txt")));
			try {
				for (int npcId = 0; npcId < Utils.getNPCDefinitionsSize(); npcId++) {
					...
					int attackAnim = line == null ? cDefs.getAttackEmote() : Integer.parseInt(getData(line, "attack", 3, 2));
					if (attackAnim == -1)
						attackAnim = line == null ? -1 : Integer.parseInt(getData(line, "range", 3, 2));
					int defenceAnim = defenceAnims.containsKey(npcId) ? defenceAnims.get(npcId) : -1;
					int deathAnim = line == null ? cDefs.getDeathEmote() : Integer.parseInt(getData(line, "death", 3, 2));
					int attackGFX = attackGFXs.containsKey(npcId) ? attackGFXs.get(npcId) : -1;
					int attackProjectile = attackProjectiles.containsKey(npcId) ? attackProjectiles.get(npcId) : -1;
